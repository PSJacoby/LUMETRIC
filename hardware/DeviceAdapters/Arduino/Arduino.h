...
//   Added paused_ field and IsPaused()/SetPaused()/OnPaused() to CArduinoHub
//   so that LUMETRIC can freeze the monitor thread during acquisition.
//   Added IsHubPaused() to CArduinoInput for use by the monitor thread.
...
public:
   ...
   int OnPaused(MM::PropertyBase* pPropt, MM::ActionType eAct); // LUMETRIC

   unsigned int GetMaxNumPatterns() { return maxNumPatterns_; }
   ...
   bool IsPaused() const { return paused_; }       // LUMETRIC
   void SetPaused(bool p) { paused_ = p; }         // LUMETRIC
...
private:
   ...
   bool paused_;           // LUMETRIC: when true, monitor thread polling is skipped
   int version_;
...
public:
   ...
   bool IsHubPaused() const; // LUMETRIC: used by monitor thread to skip polling
