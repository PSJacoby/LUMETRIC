...
//-----------------------------------------------------------------------------
// DESCRIPTION:   Arduino adapter. Needs accompanying firmware to be installed
//                on the board.
...
//
// LUMETRIC modifications:
//   - paused_ member + "Paused" property on Hub (A1-A3, A5-A7)
//   - CArduinoInput::IsHubPaused() (A4, A8)
//   - Monitor thread skips polling while paused (A9)
//   - CArduinoShutter::WriteToPort() and CArduinoSwitch::WriteToPort()
//     return early while paused (A10-A11)
   ...
   shutterState_(0)
{
   ...
   paused_           = false; // LUMETRIC A5

   InitializeDefaultErrorMessages();

   SetErrorText(ERR_PORT_OPEN_FAILED, "Failed opening Arduino USB device");
   SetErrorText(ERR_BOARD_NOT_FOUND,
      "Did not find an Arduino board with the correct firmware. "
      "Is the Arduino board connected to this serial port?");
   SetErrorText(ERR_NO_PORT_SET,
      "Hub Device not found. The Arduino Hub device is needed to create this device");
   std::ostringstream errorText;
   errorText << "The firmware version on the Arduino is not compatible with this adapter. "
...
MM::DeviceDetectionStatus CArduinoHub::DetectDevice(void)
{
   ...
   try
   {
         ...
         && 0 != portLowerCase.compare("unknown"))
      {
         ...
         // device specific default communication parameters for Arduino Duemilanova
         ...
         pS->Initialize();
         // The first second or so after opening the serial port, the Arduino is waiting
         // for firmware upgrades. Simply sleep 2 seconds.
...
int CArduinoHub::Initialize()
{
   ...
   // The first second or so after opening the serial port, the Arduino is waiting
   // for firmware upgrades. Simply sleep 1 second.
   CDeviceUtils::SleepMs(2000);
   ...
   // LUMETRIC A6: Register "Paused" property so BeanShell can freeze the monitor thread
   CPropertyAction* pActPaused = new CPropertyAction(this, &CArduinoHub::OnPaused);
   ret = CreateStringProperty("Paused", "0", false, pActPaused);
   if (ret != DEVICE_OK)
      return ret;
   AddAllowedValue("Paused", "0");
   AddAllowedValue("Paused", "1");
...
// LUMETRIC A7: property handler for "Paused"
int CArduinoHub::OnPaused(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::AfterSet)
   {
      std::string val;
   ...
   busy_(false)
{
   ...
   // Description
   int ret = CreateProperty(MM::g_Keyword_Description, "Arduino digital output driver",
      MM::String, true);
...
int CArduinoSwitch::WriteToPort(long value)
{
   ...
   // LUMETRIC A11: skip writes while hub is paused (LUMETRIC owns the serial port)
   if (hub_->IsPaused()) return DEVICE_OK;
   ...
   gateOpen_(true)
{
   ...
   // Description
   int nRet = CreateProperty(MM::g_Keyword_Description, "Arduino DAC driver",
      MM::String, true);
...
int CArduinoShutter::WriteToPort(long value)
{
   ...
   // LUMETRIC A10: skip writes while hub is paused (LUMETRIC owns the serial port)
   if (hub_->IsPaused()) return DEVICE_OK;
...
// LUMETRIC A8: returns true when the hub has been paused (LUMETRIC owns serial)
bool CArduinoInput::IsHubPaused() const
{
   return hub_ != nullptr && hub_->IsPaused();
}
...
int CArduinoMagnifier::Initialize()
{
   ...
   for (long state = 0; state < (long)magnifications_.size(); state++)
   {
      CPropertyActionEx* pActEx = new CPropertyActionEx(this,
         &CArduinoMagnifier::OnSetMagnification, state);
      std::ostringstream propName;
...
int CArduinoMagnifier::OnNumberOfMagnifications(MM::PropertyBase* pProp,
   MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
...
int CArduinoMagnifier::OnSetMagnification(MM::PropertyBase* pProp,
   MM::ActionType eAct, long state)
{
   if (eAct == MM::BeforeGet)
   {
      double mag = 1.0;
...
int ArduinoInputMonitorThread::svc()
{
   while (!stop_)
   {
      // LUMETRIC A9: while the hub is paused, LUMETRIC owns the serial port —
      // skip all polling to avoid collisions with acquisition traffic.
      if (aInput_.IsHubPaused()) {
